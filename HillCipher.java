...
public class HillCipher {
    // key matrix (2x2)
    static int[][] K = { {3, 3}, {2, 5} };           // det = 9 mod 26, invertible
    // inverse of K modulo 26
    static int[][] Kinv = { {15, 17}, {20, 9} };     // pre-computed

    // A=0, B=1, ..., Z=25
    static int c2i(char c) { return c - 'A'; }
    static char i2c(int x) { return (char) ('A' + x); }

    static String clean(String s) {
        s = s.toUpperCase().replaceAll("[^A-Z]", "");
        if (s.length() % 2 != 0) s += "X";           // pad if odd length
        return s;
    }

    static String hill(String text, int[][] M) {
        text = clean(text);
        StringBuilder out = new StringBuilder();
        for (int i = 0; i < text.length(); i += 2) {
            int p1 = c2i(text.charAt(i));
            int p2 = c2i(text.charAt(i + 1));
            int c1 = (M[0][0]*p1 + M[0][1]*p2) % 26;
            int c2 = (M[1][0]*p1 + M[1][1]*p2) % 26;
            out.append(i2c(c1)).append(i2c(c2));
        }
        return out.toString();
    }
    ...
    public static void main(String[] args) {
        ...
        String cipher = hill(plain, K);
        String decrypted = hill(cipher, Kinv);

        System.out.println("Plaintext : " + clean(plain));
